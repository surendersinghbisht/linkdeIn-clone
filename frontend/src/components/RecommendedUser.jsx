...
const RecommendedUser = ({ user }) => {
    ...
    staleTime: 0, // This ensures the query is considered stale immediately
  ...
  const { mutate: acceptRequest } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["connectionStatus", user._id] });
      queryClient.refetchQueries({ queryKey: ["connectionStatus", user._id] }); // Explicitly refetch after invalidation
    },
    onError: (error) => {
      toast.error(error.response?.data?.error || "An error occurred");
    },
